class Account
  ...
  def destroy_account
    ...
    accounts.reject! { |account| account.login == self.login }
  ...
  def save_changed_accounts
    ...
    @storage.load_accounts.each do |account|
      account.login == self.login ? accounts.push(self) : accounts.push(account)
    end
  ...
  def create_card(type)
    case type
    when CreditCard::CARD_TYPES[:usual] then self.cards << Usual.new
    when CreditCard::CARD_TYPES[:capitalist] then self.cards << Capitalist.new
    when CreditCard::CARD_TYPES[:virtual] then self.cards << Virtual.new
    end
